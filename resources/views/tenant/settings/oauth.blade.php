...
@section('title', 'OAuth Settings - Social Login')
        ...
        <div class="bg-white rounded-lg shadow-lg overflow-hidden">
            <div class="bg-gradient-to-r from-red-500 to-red-600 px-6 py-4">
                <div class="flex items-center justify-between">
                    ...
                    @elseif($googleProvider->isConfigured())
            ...
            <form action="{{ route('tenant.settings.oauth.google.update') }}" method="POST" class="p-6">
                @csrf
                @method('PUT')
                ...
                <div class="space-y-4">
                    <div>
                        ...
                        <input type="text" name="client_id"
                               value="{{ $googleProvider->client_id }}"
                    ...
                    <div>
                        ...
                        <input type="password" name="client_secret"
                               class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-red-500 focus:border-transparent"
                               placeholder="{{ $googleProvider->client_secret ? '••••••••••••••••' : 'GOCSPX-xxxxxxxxxxxxx' }}">
                    </div>
                    ...
                    <div>
                        ...
                        <div class="flex items-center space-x-2">
                            <input type="text" readonly
                                   value="{{ $googleProvider->getCallbackUrl() }}"
                    ...
                    <div class="flex items-center">
                        <input type="checkbox" name="is_active" id="google_active" value="1"
                               {{ $googleProvider->is_active ? 'checked' : '' }}
                               {{ !$googleProvider->isConfigured() ? 'disabled' : '' }}
                    ...
                    @if($googleProvider->test_status)
                        <div class="p-3 rounded-lg {{ $googleProvider->test_status == 'success' ? 'bg-green-50 text-green-700' : 'bg-red-50 text-red-700' }}">
                            <p class="text-sm font-medium">
                                Test Status: {{ ucfirst($googleProvider->test_status) }}
                            </p>
                            @if($googleProvider->test_message)
                                <p class="text-xs mt-1">{{ $googleProvider->test_message }}</p>
                            @endif
                            @if($googleProvider->last_tested_at)
                                <p class="text-xs mt-1 text-gray-500">
                                    Testato: {{ $googleProvider->last_tested_at->diffForHumans() }}
                                </p>
                            @endif
                        </div>
                    @endif
                    ...
                    <div class="flex items-center space-x-3">
                        ...
                        @if($googleProvider->isConfigured())
                            ...
                            <a href="{{ route('tenant.settings.oauth.clear', 'google') }}"
        ...
        <div class="bg-white rounded-lg shadow-lg overflow-hidden">
            <div class="bg-gradient-to-r from-blue-600 to-blue-700 px-6 py-4">
                <div class="flex items-center justify-between">
                    ...
                    @elseif($facebookProvider->isConfigured())
                        <span class="px-3 py-1 bg-yellow-100 text-yellow-800 text-xs font-semibold rounded-full">
                            <i class="fas fa-pause-circle mr-1"></i> Configurato
                        </span>
                    @else
            ...
            <form action="{{ route('tenant.settings.oauth.facebook.update') }}" method="POST" class="p-6">
                @csrf
                @method('PUT')
                ...
                <div class="space-y-4">
                    <div>
                        ...
                        <input type="text" name="client_id"
                               value="{{ $facebookProvider->client_id }}"
                    ...
                    <div>
                        ...
                        <input type="password" name="client_secret"
                               class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-600 focus:border-transparent"
                               placeholder="{{ $facebookProvider->client_secret ? '••••••••••••••••' : 'xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx' }}">
                    </div>
                    ...
                    <div>
                        ...
                        <div class="flex items-center space-x-2">
                            <input type="text" readonly
                                   value="{{ $facebookProvider->getCallbackUrl() }}"
                                   class="flex-1 px-4 py-2 bg-gray-50 border border-gray-300 rounded-lg text-gray-600"
                    ...
                    <div class="flex items-center">
                        <input type="checkbox" name="is_active" id="facebook_active" value="1"
                               {{ $facebookProvider->is_active ? 'checked' : '' }}
                               {{ !$facebookProvider->isConfigured() ? 'disabled' : '' }}
                    ...
                    @if($facebookProvider->test_status)
                        <div class="p-3 rounded-lg {{ $facebookProvider->test_status == 'success' ? 'bg-green-50 text-green-700' : 'bg-red-50 text-red-700' }}">
                            <p class="text-sm font-medium">
                                Test Status: {{ ucfirst($facebookProvider->test_status) }}
                            </p>
                            @if($facebookProvider->test_message)
                                <p class="text-xs mt-1">{{ $facebookProvider->test_message }}</p>
                            @endif
                            @if($facebookProvider->last_tested_at)
                                <p class="text-xs mt-1 text-gray-500">
                                    Testato: {{ $facebookProvider->last_tested_at->diffForHumans() }}
                                </p>
                            @endif
                        </div>
                    @endif
                    ...
                    <div class="flex items-center space-x-3">
                        ...
                        @if($facebookProvider->isConfigured())
                            ...
                            <a href="{{ route('tenant.settings.oauth.clear', 'facebook') }}"
...
function testOAuth(provider) {
    fetch(`{{ url('/dashboard/settings/oauth/test') }}/${provider}`, {
        method: 'POST',
        headers: {
