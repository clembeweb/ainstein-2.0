...
@section('content')
<div class="py-12" x-data="{ activeTab: '{{ $activeTab }}' }">
    <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
                ...
                <nav class="-mb-px flex space-x-8 px-6" aria-label="Tabs">
                    <a href="?tab=pages"
                       ...
                       class="whitespace-nowrap py-4 px-1 border-b-2 font-medium text-sm cursor-pointer">
                        ...
                        Pages
                        <span class="ml-2 py-0.5 px-2 rounded-full text-xs font-medium bg-gray-100">{{ $pages->total() }}</span>
                    </a>
                    <a href="?tab=generations"
                       ...
                       class="whitespace-nowrap py-4 px-1 border-b-2 font-medium text-sm cursor-pointer">
                        ...
                        Generations
                        <span class="ml-2 py-0.5 px-2 rounded-full text-xs font-medium bg-gray-100">{{ $generations->total() }}</span>
                    </a>
                    <a href="?tab=prompts"
                       ...
                       class="whitespace-nowrap py-4 px-1 border-b-2 font-medium text-sm cursor-pointer">
                        ...
                        Prompts
                        <span class="ml-2 py-0.5 px-2 rounded-full text-xs font-medium bg-gray-100">{{ $prompts->total() }}</span>
                    </a>
                </nav>
            </div>
        </div>
        ...
        <div x-show="activeTab === 'pages'" x-cloak>
            @include('tenant.content-generator.tabs.pages', ['pages' => $pages, 'statuses' => $statuses, 'categories' => $categories])
        </div>

        <!-- Generations Tab -->
        <div x-show="activeTab === 'generations'" x-cloak>
            @include('tenant.content-generator.tabs.generations', ['generations' => $generations, 'generationStatuses' => $generationStatuses])
        </div>

        <!-- Prompts Tab -->
        <div x-show="activeTab === 'prompts'" x-cloak>
            @include('tenant.content-generator.tabs.prompts', ['prompts' => $prompts, 'promptCategories' => $promptCategories])
        </div>
    </div>
</div>
