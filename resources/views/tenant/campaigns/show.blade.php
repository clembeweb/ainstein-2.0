    ...
    <div class="bg-white rounded-lg shadow-sm p-6 mb-6">
        <div class="flex items-center justify-between mb-4">
            <div class="flex-1">
                ...
                <p class="text-gray-600">{{ $campaign->info }}</p>
            </div>
        </div>
        ...
        <div class="flex flex-wrap gap-2 mt-6 pt-6 border-t">
            ...
            <a href="{{ route('tenant.campaigns.edit', $campaign->id) }}"
            ...
            @if($campaign->assets->count() > 0)
                <div class="relative" x-data="{ open: false }">
                    ...
                    <div x-show="open"
                        ...
                        <a href="{{ route('tenant.campaigns.export', ['id' => $campaign->id, 'format' => 'csv']) }}"
                        ...
                        <a href="{{ route('tenant.campaigns.export', ['id' => $campaign->id, 'format' => 'google-ads']) }}"
                           class="flex items-center px-4 py-2 text-sm text-gray-700 hover:bg-gray-100">
            ...
            <form method="POST" action="{{ route('tenant.campaigns.destroy', $campaign->id) }}" class="inline ml-auto">
                @csrf
                @method('DELETE')
                <button type="submit"
            ...
            <div class="bg-gradient-to-br from-green-50 to-green-100 rounded-lg p-4 border border-green-200">
                <div class="flex items-center">
                    ...
                    <div class="ml-4">
                        ...
                        <p class="text-2xl font-bold text-green-900">{{ $campaign->assets->count() }}</p>
                    </div>
                </div>
            </div>
            ...
            <div class="bg-gradient-to-br from-amber-50 to-amber-100 rounded-lg p-4 border border-amber-200">
                <div class="flex items-center">
                    ...
                    <div class="ml-4">
                        ...
                        <p class="text-2xl font-bold text-amber-900">{{ number_format($campaign->tokens_used ?? 0) }}</p>
                    </div>
                </div>
            </div>
            ...
            <div class="bg-gradient-to-br from-blue-50 to-blue-100 rounded-lg p-4 border border-blue-200">
                <div class="flex items-center">
                    ...
                    <div class="ml-4">
                        ...
                        <p class="text-lg font-bold text-blue-900">{{ $campaign->created_at->format('d/m/Y') }}</p>
                        <p class="text-xs text-blue-600">{{ $campaign->created_at->format('H:i') }}</p>
                    </div>
                </div>
            </div>
        </div>
                ...
                <div class="flex flex-wrap gap-2">
                    @foreach($keywordsArray as $keyword)
                        <span class="inline-flex items-center px-3 py-1 rounded-full text-sm font-medium bg-blue-50 text-blue-700 border border-blue-200">
                            <i class="fas fa-tag mr-2 text-xs text-blue-500"></i>{{ $keyword }}
                        </span>
                    @endforeach
                </div>
            </div>
                                                ...
                                                <div class="flex-1">
                                                    <div class="flex items-center mb-2">
                                                        ...
                                                        <span class="text-xs font-medium text-gray-500">
                                                            {{ mb_strlen($title) }}/30 caratteri
                                                        </span>
                                                    </div>
                                                    <p class="text-gray-900 font-medium ml-9">{{ $title }}</p>
                                                </div>
                                                <button @click="copyToClipboard('{{ addslashes($title) }}', 'title-{{ $index }}')"
                                                        class="ml-4 px-3 py-2 text-blue-600 hover:bg-blue-100 rounded-lg transition-colors">
                                                    <i class="fas" :class="copiedId === 'title-{{ $index }}' ? 'fa-check' : 'fa-copy'"></i>
                                                </button>
                                            </div>
                                        </div>
                                    @endforeach
                        ...
                        @if(!empty($asset->descriptions))
                            <div>
                                <h4 class="text-lg font-semibold text-gray-900 mb-4 flex items-center">
                                    ...
                                    Descrizioni ({{ count($asset->descriptions) }})
                                </h4>
                                <div class="grid gap-3">
                                    @foreach($asset->descriptions as $index => $description)
                                            ...
                                            <div class="flex items-start justify-between">
                                                <div class="flex-1">
                                                    <div class="flex items-center mb-2">
                                                        ...
                                                        <span class="text-xs font-medium text-gray-500">
                                                            {{ mb_strlen($description) }}/90 caratteri
                                                        </span>
                                                    </div>
                                                    <p class="text-gray-900 ml-9">{{ $description }}</p>
                                                </div>
                                                <button @click="copyToClipboard('{{ addslashes($description) }}', 'desc-{{ $index }}')"
                                                        class="ml-4 px-3 py-2 text-green-600 hover:bg-green-100 rounded-lg transition-colors">
                                                    <i class="fas" :class="copiedId === 'desc-{{ $index }}' ? 'fa-check' : 'fa-copy'"></i>
                                                </button>
                                            </div>
                                        </div>
                                    @endforeach
                        ...
                        @if(!empty($asset->titles))
                            <div>
                                <h4 class="text-lg font-semibold text-gray-900 mb-4 flex items-center">
                                    ...
                                    Titoli Brevi ({{ count($asset->titles) }})
                                </h4>
                                <div class="grid gap-3">
                                    @foreach($asset->titles as $index => $title)
                                        <div class="border border-gray-200 rounded-lg p-4 hover:border-blue-300 hover:bg-blue-50 transition-all">
                                            <div class="flex items-start justify-between">
                                                ...
                                                <button @click="copyToClipboard('{{ addslashes($title) }}', 'short-{{ $index }}')"
                                                        class="ml-4 px-3 py-2 text-blue-600 hover:bg-blue-100 rounded-lg transition-colors">
                                                    <i class="fas" :class="copiedId === 'short-{{ $index }}' ? 'fa-check' : 'fa-copy'"></i>
                                                </button>
                                            </div>
                                        </div>
                                    @endforeach
                                            ...
                                            <div class="flex items-start justify-between">
                                                <div class="flex-1">
                                                    <div class="flex items-center mb-2">
                                                        ...
                                                        <span class="text-xs font-medium text-gray-500">
                                                            {{ mb_strlen($longTitle) }}/90 caratteri
                                                        </span>
                                                    </div>
                                                    <p class="text-gray-900 font-medium ml-9">{{ $longTitle }}</p>
                                                </div>
                                                <button @click="copyToClipboard('{{ addslashes($longTitle) }}', 'long-{{ $index }}')"
                                                        class="ml-4 px-3 py-2 text-purple-600 hover:bg-purple-100 rounded-lg transition-colors">
                                                    <i class="fas" :class="copiedId === 'long-{{ $index }}' ? 'fa-check' : 'fa-copy'"></i>
                                                </button>
                                            </div>
                                        </div>
                                    @endforeach
                                    ...
                                    @foreach($asset->descriptions as $index => $description)
                                        <div class="border border-gray-200 rounded-lg p-4 hover:border-green-300 hover:bg-green-50 transition-all">
                                            <div class="flex items-start justify-between">
                                                ...
                                                <button @click="copyToClipboard('{{ addslashes($description) }}', 'pmax-desc-{{ $index }}')"
                                                        class="ml-4 px-3 py-2 text-green-600 hover:bg-green-100 rounded-lg transition-colors">
                                                    <i class="fas" :class="copiedId === 'pmax-desc-{{ $index }}' ? 'fa-check' : 'fa-copy'"></i>
                                                </button>
                                            </div>
                                        </div>
                                    @endforeach
        ...
        <div x-show="activeTab === 'details'" x-transition>
            <div class="space-y-4">
                <div class="border-b pb-4">
                    ...
                    <p class="mt-1 text-gray-900">{{ $campaign->name }}</p>
                </div>
                ...
                <div class="border-b pb-4">
                    ...
                    <p class="mt-1 text-gray-900">{{ $campaign->info }}</p>
                </div>
                ...
                @if($campaign->url)
                    <div class="border-b pb-4">
                        ...
                        <p class="mt-1">
                            <a href="{{ $campaign->url }}" target="_blank" class="text-blue-600 hover:text-blue-800 hover:underline">
                                {{ $campaign->url }} <i class="fas fa-external-link-alt text-xs ml-1"></i>
                            </a>
                        </p>
                    </div>
                @endif
                ...
                <div class="border-b pb-4">
                    ...
                    <p class="mt-1 text-gray-900">{{ strtoupper($campaign->language ?? 'it') }}</p>
                </div>

                <div class="border-b pb-4">
                    <label class="text-sm font-semibold text-gray-700">Data Creazione</label>
                    <p class="mt-1 text-gray-900">{{ $campaign->created_at->format('d/m/Y H:i') }}</p>
                </div>

                <div>
                    <label class="text-sm font-semibold text-gray-700">Ultimo Aggiornamento</label>
                    <p class="mt-1 text-gray-900">{{ $campaign->updated_at->format('d/m/Y H:i') }}</p>
                </div>
            </div>
        </div>
    </div>
    ...
    <div x-show="showRegenerateModal"
        ...
        <div @click.away="showRegenerateModal = false"
            ...
            <div class="flex justify-end space-x-3 mt-6 pt-4 border-t">
                ...
                <form method="POST" action="{{ route('tenant.campaigns.regenerate', $campaign->id) }}" class="inline">
                    @csrf
                    <button type="submit"
